...
class GTFSLoadTest(unittest.TestCase):
    ...
    def test_query_trips_info_by_area(self):
        result = list(query_trips_info_by_area(session=self.session, line_string_2pt=area_query_linestring))
    ...
    def test_query_stops_info_by_path(self):
        result = list(query_stops_info_by_path(session=self.session, line_string_path=path_query_linestring))
        self.assertEqual(3, len(result),
                         'the result count size in the db don\'t match the result in the query')

    def test_query_stoptimes_info_by_area(self):
        result = list(query_stoptimes_info_by_area(session=self.session, line_string_2pt=area_query_linestring))
        self.assertEqual(2180, len(result),
                         'the result count size in the db don\'t match the result in the query')

    def test_query_stoptimes_info_by_path(self):
        result = list(query_stoptimes_info_by_path(session=self.session, line_string_path=path_query_linestring))
